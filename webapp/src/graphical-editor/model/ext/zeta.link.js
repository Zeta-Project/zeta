...
joint.shapes.zeta.MLinkView = joint.dia.LinkView.extend({
    ...
    render: function () {
        this.$el.empty();
        var children = joint.V(this.model.get('markup') || this.model.markup);
        ...
        joint.V(this.el).append(children);
    ...
    renderPlacings: function () {
        ...
        var placingNodeInstance = joint.V(placingTemplate());
        ...
        _.each(placings, function (placing, idx) {
            ...
            placingNode.append(joint.V(placing.markup).attr(placing.attrs));
    ...
    rotateAndTranslatePlacing: function (placing, position, reference, target) {
        var angle = joint.g.point(position).changeInAngle(position.x - reference.x, position.y - reference.y, reference);
        placing.rotate(angle, 0, 0);

        var bbox = placing.bbox(false, target);
        var finalPosition = joint.g.point(position).move(reference, bbox.width / 2);
...
const MLink = joint.shapes.zeta.MLink;
const MLinkView = joint.shapes.zeta.MLinkView;
export { MLink, MLinkView };