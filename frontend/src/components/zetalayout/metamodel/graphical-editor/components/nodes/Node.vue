    ...
    <svg :width="layout.width" :height="layout.height">
      ...
      <g :style="{ fontSize: '10px', fontColor: 'white', fontFamily: 'Roboto,sans-serif', fontWeight: 300, fill: (tag.abstractness ? 'rgb(220, 20, 60)' :'rgb(96,125,139)' )}">
        ...
        <g>
          ...
          <g>
            <image v-if="!attributes_open" :x="0" :y="50" v-on:click="attributes_open=change_status(attributes_open)"
                   xlink:href="../../assets/triangle.svg"/>
            <image v-if="attributes_open" :x="0" :y="50" v-on:click="attributes_open=change_status(attributes_open)"
                   xlink:href="../../assets/triangle_90deg_rotated.svg"/>
          </g>
          <text x="20" y="70" :style="{ fontSize:'16px', fill: 'black' }">Attributes:</text>
          <image :x="layout.width - 22" :y="55" :width="18" xlink:href="../../assets/add-sign.svg"
        ...
        <g>
          <rect x="1" :y="82 + 30 * (attributes_open ? Object.keys(tag.attributes).length : 0)" width="100%"
                :height="30"/>
          <g v-on:click="operations_open=change_status(operations_open)">
            <image v-if="!operations_open" :x="0"
                   :y="82 + 30 * (attributes_open ? Object.keys(tag.attributes).length : 0)"
                   xlink:href="../../assets/triangle.svg"/>
            <image v-if="operations_open" :x="0"
                   :y="82 + 30 * (attributes_open ? Object.keys(tag.attributes).length : 0)"
                   xlink:href="../../assets/triangle_90deg_rotated.svg"/>
          ...
          <text x="20" :y="102 + 30 * (attributes_open ? Object.keys(tag.attributes).length : 0)"
                :style="{  fontSize: '16px', fill: 'black' }">Methods:
          </text>
