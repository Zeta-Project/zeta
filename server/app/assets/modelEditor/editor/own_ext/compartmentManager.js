var compartmentManager = (function compartmentManager () {
    'use strict';
            ...
            if (cellViewsBelow.length) {
                ...
                if(cellViewBelow) {
                    ...
                    cellViewBelow.model.get('compartments').forEach(function (comp) {
                        ...
                        var cursor = g.point(x,y);
                        // rotate cursor if needed
                        if(rotation.angle != 0) {
                            cursor = cursor.rotate(g.point(rotation.cx, rotation.cy), rotation.angle);
                        }
                        var rect = g.rect(scaledElement.x, scaledElement.y, scaledElement.width, scaledElement.height);
