...
function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
	...
	var matcherCachedRuns = 0,
		...
		superMatcher = function( seed, context, xml, results, expandContext ) {
			...
			// Override datavis of globals by nested matchers
			if ( outermost ) {
				dirruns = dirrunsUnique;
	...
	remove: function( owner, key ) {
		...
		if ( key === undefined ) {
			...
			if ( jQuery.isArray( key ) ) {
				...
				// Try the string as a key before any datavis
				if ( key in cache ) {
					name = [ key, camel ];
				} else {
...
;(function() {
  ...
  function runInContext(context) {
	 ...
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ];
	 *  // using "_.pluck" callback shorthand _.every(stooges, 'age'); // =>
	 * true
	 *  // using "_.where" callback shorthand _.every(stooges, { 'age': 50 }); // =>
	 * false
	 */
	 ...
	 *  // using "_.pluck" callback shorthand _.filter(food, 'organic'); // => [{
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' }]
	 *  // using "_.where" callback shorthand _.filter(food, { 'type': 'fruit'
	 ...
	 * 'organic': false, 'type': 'vegetable' } ];
	 *  // using "_.where" callback shorthand _.find(food, { 'type': 'vegetable'
	 * }); // => { 'name': 'beet', 'organic': false, 'type': 'vegetable' }
	 *  // using "_.pluck" callback shorthand _.find(food, 'organic'); // => {
	 ...
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ];
	 *  // using "_.pluck" callback shorthand _.map(stooges, 'name'); // =>
	 * ['moe', 'larry']
	 */
	 ...
	 * 'larry', 'age': 50 };
	 *  // using "_.pluck" callback shorthand _.max(stooges, 'age'); // => {
	 * 'name': 'larry', 'age': 50 };
	 */
	 ...
	 * 'moe', 'age': 40 };
	 *  // using "_.pluck" callback shorthand _.min(stooges, 'age'); // => {
	 * 'name': 'moe', 'age': 40 };
	 */
	 ...
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' } ];
	 *  // using "_.pluck" callback shorthand _.reject(food, 'organic'); // => [{
	 * 'name': 'apple', 'organic': false, 'type': 'fruit' }]
	 *  // using "_.where" callback shorthand _.reject(food, { 'type': 'fruit'
	 * }); // => [{ 'name': 'carrot', 'organic': true, 'type': 'vegetable' }]
	 */
	 ...
	 * var food = [ { 'name': 'apple', 'organic': false, 'type': 'fruit' }, {
	 * 'name': 'carrot', 'organic': true, 'type': 'vegetable' } ];
	 *  // using "_.pluck" callback shorthand _.some(food, 'organic'); // =>
	 * true
	 *  // using "_.where" callback shorthand _.some(food, { 'type': 'meat' }); // =>
	 * false
	 */
	 ...
	 * [3, 1, 2]
	 *  // using "_.pluck" callback shorthand _.sortBy(['banana', 'strawberry',
	 * 'apple'], 'length'); // => ['apple', 'banana', 'strawberry']
	 */
	 ...
	 * 'organic': false }, ];
	 *  // using "_.pluck" callback shorthand _.first(food, 'organic'); // => [{
	 * 'name': 'banana', 'organic': true }]
	 * 
	 * var food = [ { 'name': 'apple', 'type': 'fruit' }, { 'name': 'banana',
	 * 'type': 'fruit' }, { 'name': 'beet', 'type': 'vegetable' } ];
	 *  // using "_.where" callback shorthand _.first(food, { 'type': 'fruit'
	 * }); // => [{ 'name': 'apple', 'type': 'fruit' }, { 'name': 'banana',
	 * 'type': 'fruit' }]
	 */
	 ...
	 * knucklehead!'] } ];
	 *  // using "_.pluck" callback shorthand _.flatten(stooges, 'quotes'); // =>
	 ...
	 * 'organic': true } ];
	 *  // using "_.pluck" callback shorthand _.initial(food, 'organic'); // => [{
	 * 'name': 'beet', 'organic': false }]
	 * 
	 * var food = [ { 'name': 'banana', 'type': 'fruit' }, { 'name': 'beet',
	 * 'type': 'vegetable' }, { 'name': 'carrot', 'type': 'vegetable' } ];
	 *  // using "_.where" callback shorthand _.initial(food, { 'type':
	 * 'vegetable' }); // => [{ 'name': 'banana', 'type': 'fruit' }]
	 */
	 ...
	 * var food = [ { 'name': 'beet', 'organic': false }, { 'name': 'carrot',
	 * 'organic': true } ];
	 *  // using "_.pluck" callback shorthand _.last(food, 'organic'); // => [{
	 * 'name': 'carrot', 'organic': true }]
	 * 
	 * var food = [ { 'name': 'banana', 'type': 'fruit' }, { 'name': 'beet',
	 * 'type': 'vegetable' }, { 'name': 'carrot', 'type': 'vegetable' } ];
	 *  // using "_.where" callback shorthand _.last(food, { 'type': 'vegetable'
	 * }); // => [{ 'name': 'beet', 'type': 'vegetable' }, { 'name': 'carrot',
	 * 'type': 'vegetable' }]
	 */
	 ...
	 * var food = [ { 'name': 'banana', 'organic': true }, { 'name': 'beet',
	 * 'organic': false }, ];
	 *  // using "_.pluck" callback shorthand _.rest(food, 'organic'); // => [{
	 * 'name': 'beet', 'organic': false }]
	 * 
	 * var food = [ { 'name': 'apple', 'type': 'fruit' }, { 'name': 'banana',
	 * 'type': 'fruit' }, { 'name': 'beet', 'type': 'vegetable' } ];
	 *  // using "_.where" callback shorthand _.rest(food, { 'type': 'fruit' }); // => [{
	 ...
	 * 'curly', 'age': 60 } };
	 *  // modifying the result cache var stooge = _.memoize(function(name) {
	 * return data[name]; }, _.identity); stooge('curly'); // => { 'name':
	 * 'curly', 'age': 60 }
	 * 
	 ...
	 * "interpolate" delimiter _.template('hello ${ name }', { 'name': 'curly'
	 * }); // => 'hello curly'
	 ...
	 * var stooges = [ { 'name': 'moe', 'age': 40 }, { 'name': 'larry', 'age':
	 * 50 } ];
	 *  // without explicit chaining _(stooges).first(); // => { 'name': 'moe',
	 * 'age': 40 }
	 *  // with explicit chaining _(stooges).chain() .first() .pick('age')
	 * .value() // => { 'age': 40 }
	 */
...
joint.dia.CellView = Backbone.View.extend({
    ...
    highlight: function(el, color) {
        ...
        if (color) {
        	V(el).addClass('highlighted-' + color);
        }
    },

    unhighlight: function(el, color) {

        el = !el ? this.el : this.$(el)[0] || this.el;
        
        V(el).removeClass('highlighted');
        if (color) {
        	V(el).removeClass('highlighted-' + color);
        }
    },
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    isAnyParentPaperScroller: function(element){
                ...
                if (this._targetEvent !== target) {
                    ...
                    if (this._viewUnderPointer) {
                        ...
                        if (this._magnetUnderPointer && this.paper.options.validateConnection.apply(
                            ...
                            contextMenu.focusElement(this.model.id,
                                                     this._viewUnderPointer.model.attributes.ecoreName,
                                                     document.elementFromPoint(evt.clientX, evt.clientY),
                                                     evt.clientX,
                                                     evt.clientY);
                        ...
                        contextMenu.lostFocus(document.elementFromPoint(evt.clientX, evt.clientY), evt.clientX, evt.clientY);
...
if (typeof exports === 'object') {
    ...
    var joint = {
        ...
        shapes: {},
        dia: require('../src/joint.dia.element')
    };
    var _ = require('lodash');
}
...
joint.shapes.basic.TextBlockView = joint.dia.ElementView.extend({
    ...
    initialize: function() {
        ...
        if (typeof SVGForeignObjectElement === 'undefined') {
            ...
            this.listenTo(this.model, 'change:size change:content', this.displayContent);
        }
    },
    ...
    render: function() {
        ...
        if (typeof SVGForeignObjectElement === 'undefined') {
            ...
            this.displayContent(this.model);
        }
    },
