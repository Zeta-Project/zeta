...
import models.document.{ ModelEntity, Repository => Documents }
...
trait Transformer {
  def prepare(model: String)(implicit documents: Documents, files: Files, remote: Remote): Future[Transformer] = Future.successful(this)

  def prepare(models: List[String])(implicit documents: Documents, files: Files, remote: Remote): Future[Transformer] = Future.successful(this)
