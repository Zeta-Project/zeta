...
//    """.stripMargin
  ...
  "A StyleParser" should "succeed" in {
    ...
    assert(style.attributes(1).asInstanceOf[BackgroundColor].color == "white")

    assert(style.attributes(2).attributeName == "line-color")
    assert(style.attributes(2).asInstanceOf[LineColor].color == "black")
    ...
    assert(style.attributes(5).asInstanceOf[FontColor].color == "black")
    ...
    assert(style.attributes(11).asInstanceOf[GradientAreaColor].color == "black")
...
// todo: this test fails: missing implementation
