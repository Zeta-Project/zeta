...
trait UnorderedParser extends JavaTokenParsers {
  ...
  // - range(_, y), y > 1
  // - exact(x), x > 1
  def *[T](implicit list: List[Any], classTag: ClassTag[T]): List[T] = list.collect { case x: T => x }

  //noinspection ScalaStyle
  // get single value by type (optional) => use for ParserConf like
  // - optional()
  def ?[T](implicit list: List[Any], classTag: ClassTag[T]): Option[T] = *(list, classTag).headOption

  //noinspection ScalaStyle
  // get single value by type (throws) => use for ParserConf like
  // - once()
  def !![T](implicit list: List[Any], classTag: ClassTag[T]): T = ?(list, classTag).get
