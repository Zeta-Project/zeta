...
object NodeParser extends CommonParserMethods with UniteParsers with ShapeTokens {
  ...
  def parseNode: Parser[NodeParseTree] = {
    ("node" ~> ident) ~ ("for" ~> ident) ~ leftBrace ~ edges ~ attributes ~ geoModels ~ rightBrace ^^ { parseResult =>
      ...
      NodeParseTree(nodeName, conceptElement, edges, attributes, geoModels)
    }
  }
  ...
  private def attributes = rep(style | sizeMin | sizeMax | resizing)
