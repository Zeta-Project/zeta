...
class NodesTest extends FlatSpec with Matchers {
  ...
  "generateFor" should "generate this rule from the meta model" in {
    val mClass1 = MClass("class1", abstractness = false, superTypeNames = Seq.empty, Seq.empty, Seq.empty, Seq[MAttribute]())
    val mClass2 = MClass("class2", abstractness = false, superTypeNames = Seq.empty, Seq.empty, Seq.empty, Seq[MAttribute]())
