  ...
  def triggerParse(id: UUID)(request: SecuredRequest[ZetaEnv, AnyContent]): Future[Result] =
    protectedRead(id, request, graphicalDsl => {
      graphicalDslParser.parse(
        ...
        errorResult => Ok(taskResultFormat.writes(TaskResult.error(errorResult.errorDsl, errorResult.errors, errorResult.position))),
  ...
  private def parseAndSerialize[A](id: UUID, request: SecuredRequest[ZetaEnv, A], serialize: GraphicalDsl => JsObject): Future[Result] = {
    protectedRead(id, request, (graphicalDsl: GdslProject) => {
      graphicalDslParser.parse(
        ...
        errorResult => InternalServerError(taskResultFormat.writes(TaskResult.error(errorResult.errorDsl, errorResult.errors, errorResult.position))),
