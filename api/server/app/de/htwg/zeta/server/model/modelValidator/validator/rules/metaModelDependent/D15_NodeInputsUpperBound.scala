...
class D15_NodeInputsUpperBound(nodeType: String, inputType: String, upperBound: Int) extends SingleNodeRule with DslRule {
...
object D15_NodeInputsUpperBound extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritInputs(Util.simplifyMetaModelGraph(metaModel))
    ...
    .foldLeft(Seq[DslRule]()) { (acc, currentClass) =>
      acc ++ currentClass.inputs.map(input => new D15_NodeInputsUpperBound(currentClass.name, input.name, input.upperBound))
    }
}
