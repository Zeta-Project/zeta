...
class D25_EdgeAttributeEnumTypes(edgeType: String, attributeType: String, enumName: String) extends SingleEdgeRule with DslRule {
...
object D25_EdgeAttributeEnumTypes extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.getReferences(metaModel)
    .foldLeft(Seq[DslRule]()) { (acc, currentReference) =>
      ...
      acc ++ currentReference.attributes.flatMap(att => att.`type` match {
        case attType: MEnum => Some(new D25_EdgeAttributeEnumTypes(currentReference.name, att.name, attType.name))
        case _ => None
      })
    }
}
