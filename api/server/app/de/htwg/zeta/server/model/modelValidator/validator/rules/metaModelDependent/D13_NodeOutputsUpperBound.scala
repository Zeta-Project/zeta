...
class D13_NodeOutputsUpperBound(nodeType: String, outputType: String, upperBound: Int) extends SingleNodeRule with DslRule {
  override val name: String = getClass.getSimpleName
  override val description: String = s"Nodes of type $nodeType must have a maximum of $upperBound output edges of type $outputType."
  override val possibleFix: String = s"Remove output edges of type $outputType from nodes of type $nodeType until there are a maximum of $upperBound output edges."
...
object D13_NodeOutputsUpperBound extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritOutputs(Util.simplifyMetaModelGraph(metaModel))
    ...
    .foldLeft(Seq[DslRule]()) { (acc, currentClass) =>
      acc ++ currentClass.outputs.map(output => new D13_NodeOutputsUpperBound(currentClass.name, output.name, output.upperBound))
    }
}
