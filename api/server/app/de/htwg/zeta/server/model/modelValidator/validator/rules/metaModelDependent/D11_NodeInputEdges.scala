...
class D11_NodeInputEdges(nodeType: String, inputTypes: Seq[String]) extends SingleNodeRule with DslRule {
...
object D11_NodeInputEdges extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritInputs(Util.simplifyMetaModelGraph(metaModel))
    ...
    .foldLeft(Seq[DslRule]()) { (acc, currentClass) =>
      if (currentClass.inputs.isEmpty) acc
      else acc :+ new D11_NodeInputEdges(currentClass.name, currentClass.inputs.map(_.name))
    }
}
