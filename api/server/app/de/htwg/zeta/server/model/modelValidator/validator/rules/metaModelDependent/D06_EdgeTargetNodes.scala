...
class D06_EdgeTargetNodes(edgeType: String, targetTypes: Seq[String]) extends SingleEdgeRule with DslRule {
...
object D06_EdgeTargetNodes extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.getReferences(metaModel)
    .foldLeft(Seq[DslRule]()) { (acc, currentReference) =>
      if (currentReference.target.isEmpty) acc
      else acc :+ new D06_EdgeTargetNodes(currentReference.name, currentReference.target.map(_.mType.name))
    }
}
