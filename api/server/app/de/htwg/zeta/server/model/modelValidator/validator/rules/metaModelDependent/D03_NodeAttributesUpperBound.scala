...
class D03_NodeAttributesUpperBound(nodeType: String, attributeType: String, upperBound: Int) extends SingleNodeRule with DslRule {
  override val name: String = getClass.getSimpleName
  override val description: String = s"Nodes of type $nodeType must have a maximum of $upperBound attributes of type $attributeType."
  override val possibleFix: String = s"Remove attributes of type $attributeType from nodes of type $nodeType until there are a maximum of $upperBound attributes."
...
object D03_NodeAttributesUpperBound extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritAttributes(Util.simplifyMetaModelGraph(metaModel))
    ...
    .foldLeft(Seq[DslRule]()) { (acc, currentClass) =>
      acc ++ currentClass.attributes.map(attr => new D03_NodeAttributesUpperBound(currentClass.name, attr.name, attr.upperBound))
    }
}
