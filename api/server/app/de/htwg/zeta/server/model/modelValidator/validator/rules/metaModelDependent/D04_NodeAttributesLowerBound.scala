...
class D04_NodeAttributesLowerBound(val nodeType: String, val attributeType: String, val lowerBound: Int) extends SingleNodeRule with DslRule {
...
object D04_NodeAttributesLowerBound extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritAttributes(Util.simplifyMetaModelGraph(metaModel))
    ...
    .foldLeft(Seq[DslRule]()) { (acc, currentClass) =>
      acc ++ currentClass.attributes.map(attr => new D04_NodeAttributesLowerBound(currentClass.name, attr.name, attr.lowerBound))
    }
}
