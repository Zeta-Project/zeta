...
class D19_NodesNoAttributes(nodeType: String) extends SingleNodeRule with DslRule {
...
object D19_NodesNoAttributes extends GeneratorRule {
  override def generateFor(metaModel: MetaModel): Seq[DslRule] = Util.inheritAttributes(Util.simplifyMetaModelGraph(metaModel))
    ...
    .map(cl => new D19_NodesNoAttributes(cl.name))
}
